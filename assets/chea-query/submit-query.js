...
function sliderChange(event) {
	...
	var set1Values = chea3Results[event.target.id].map(function(transcriptionFactors) {
		return transcriptionFactors.split('_')[0];
	});

	var set1ValuesSliderSubset = set1Values.splice(0, event.target.value - 1);
			...
			jQuery.get('assets/chea-query/example_results.json', function(results) {
				...
				results_div.innerHTML += captionAndTableMarkup;
		  		...
		  		for (i = 0; i < lib_names.length; i++) {
					...
					var lib_results = results.responseJSON[lib_names[i]];
